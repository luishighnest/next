...
export async function syncDaznLiveEvents() {
    try {
        const url = "https://rail-router.discovery.indazn.com/eu/v10/Rail?platform=web&id=Live&country=it&brand=dazn&languageCode=it";
        ...
        if (!resp.ok) {
            console.warn(`[Sync DAZN Live] Fetch rail Live fallito: HTTP ${resp.status}`);
            return null;
        }
        ...
        const currentLiveNorms = new Set();

        for (const t of tiles) {
            const rawTitle = (t.Title || "").trim();
            if (!rawTitle) continue;
            const normTitle = normalizeTitle(rawTitle);
            currentLiveNorms.add(normTitle);
        ...
        // Rimuovi eventi con parametri vuoti (non estratti) che non sono più tra i Live di DAZN
        for (const cat of Object.keys(updatedEventi)) {
            ...
            updatedEventi[cat] = updatedEventi[cat].filter(ev => {
                ...
                const norm = normalizeTitle(ev.name || ev.title);
                return currentLiveNorms.has(norm);
            });
