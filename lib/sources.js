...
export function getNormalizedSources(ch) {
    ...
    if (Array.isArray(ch.sources) && ch.sources.length > 0) {
        return ch.sources.map(s => {
            ...
            const isWarp = Boolean(s.isWarp || (cleaned.url && (cleaned.url.includes("@eyj") || cleaned.url.includes("token=eyj"))) || (s.name && s.name.toUpperCase().includes("WARP")));
            return {
                ...s,
                url: cleaned.url,
                kid_key: cleaned.key,
    ...
    if (ch.url) {
        ...
        const isWarp = Boolean(ch.isWarp || (cleaned.url && (cleaned.url.includes("@eyj") || cleaned.url.includes("token=eyj"))));
        return [{
